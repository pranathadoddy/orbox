...
class AgentReportIndex {
    ...
    _LoadSummary() {
        ...
        $summary.each(function (i, v) {
            ...
            $.get(`${$(v).data('url')}/${self.selectedDate.format('YYYY-MM-DD')}`, undefined, function (resp) {
                spinner.hide();
    ...
    _FetchItemListData(container, url, callback) {
        var self = this;

        $.get(url, undefined, function (resp) {
            if (resp.isSuccess) {
    ...
    _LoadItemDetailList() {
        ...
        $listItem.each(function (i, v) {
            ...
            self._FetchItemListData(v, `${$(v).data('url')}/${self.selectedDate.format('YYYY-MM-DD')}/0`, function () {
                spinner.hide();
            });
        });
    }
...
export default AgentReportIndex;